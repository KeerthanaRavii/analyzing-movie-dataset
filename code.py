...
def duplicate_and_unique_movies(dataset, index_):
    ...
    for i in range(len(dataset)):
        movie_names.append(dataset[i][13])
    movie_set=set(movie_names)
    movie_frequency=[]
    for i in movie_set:
        movie_frequency.append(movie_names.count(i))
    count=0
...
for i in range(len(movies)):
    movie_names.append(movies[i][13])
movie_set=set(movie_names)
movie_frequency=[]
for i in movie_set:
    reviews_max[i]=movie_names.count(i)
